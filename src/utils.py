...
def evaluate_models(X_train, y_train, X_test, y_test, model, parameters):
    try:
        # Instantiate model
        model_obj = model

        # Instantiate a GridSearchCV object using 5-fold CV
        gs = GridSearchCV(model_obj, parameters, cv = 5)
            
        # Perform a grid search cv
